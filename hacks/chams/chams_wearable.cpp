...
void chams_wearable(Entity* entity, void* me, void* state, ModelRenderInfo* pinfo, VMatrix* bone_to_world) {
  ...
  Entity* owner_entity = entity->get_owner_entity();
  if (owner_entity->get_class_id() != class_id::PLAYER) DME_RETURN;
  Player* owner_player = (Player*)owner_entity;
  
  
  if (owner_player == localplayer && config.chams.player.local == false) DME_RETURN; // Ignore Local Player
  if (entity->is_dormant()) DME_RETURN;
  if (entity->get_team() != localplayer->get_team() && config.chams.player.enemy == false && !owner_player->is_friend()) DME_RETURN; // Ignore Enemy
  if (entity->get_team() == localplayer->get_team() && owner_player != localplayer && config.chams.player.team == false && !owner_player->is_friend()) DME_RETURN; // Ignore Team
  if (owner_player->is_friend() && config.chams.player.friends == false && (config.chams.player.team == false && entity->get_team() == localplayer->get_team())) DME_RETURN; // Ignore Friends
  ...
  if (owner_player->get_team() != localplayer->get_team()) {
